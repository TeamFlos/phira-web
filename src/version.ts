...
export type DiffRow =
  ...
  /** Image, rendered as two thumbnails side by side; `pending` while a content hash is fetched. */
  | { field: string; kind: 'image'; changed: boolean; pending: boolean; from: string; to: string }
  /** Preview audio, rendered as two playback bars side by side; `pending` while a content hash is fetched. */
  | { field: string; kind: 'audio'; changed: boolean; pending: boolean; from: string; to: string }
  /** Opaque binary (chart archive): only "changed or not"; `pending` while a content hash is fetched. */
  | { field: string; kind: 'asset'; changed: boolean; pending: boolean; from: string; to: string };
...
export function diffVersions(from: ChartVersion, to: ChartVersion): DiffRow[] {
  ...
  return [
    ...
    // Illustration and preview are opaque server-side blobs (UUID paths). Their URLs differ
    // on every upload even when the content is identical, so URL comparison always reports a
    // change. Mark them as pending until VersionDiff fetches and compares their content hashes.
    ...
    { field: 'file', kind: 'asset', changed: from.checksum !== to.checksum, pending: false, from: a.file, to: b.file },
  ];
}
