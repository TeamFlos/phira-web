...
const props = withDefaults(defineProps<{ name: string, init?: number, canBeZero?: boolean }>(), {
  canBeZero: false,
});
...
<template>
  <div class="justify-center rating rating-lg rating-half">
    <input type="radio" :name="name" class="rating-hidden" :disabled="!canBeZero" :checked="!selected" />
