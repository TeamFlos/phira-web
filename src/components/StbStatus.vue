...
const status = (await fetchApi(
  `/chart/${props.chart}/stabilize-status`,
)) as StbStatus;
...
<template>
  <div class="px-4 flex flex-col">
    <ul class="steps steps-vertical lg:steps-horizontal my-4">
      ...
      <li
        class="step"
        :class="{ 'step-primary': status.stable }"
        v-t="'steps.done'"
      ></li>
    </ul>
    <ol
      v-if="status.history.length"
      class="relative border-l border-gray-200 dark:border-gray-700 p-2"
    >
      <li v-for="item in status.history" :key="item.id" class="mb-10 ml-4">
        ...
        >
          <UserAvatar
            ...
            :url="item.reviewerAvatar ?? uploader.avatar"
          />
        </router-link>
        <time
          class="mb-1 text-sm font-normal leading-none text-gray-400 dark:text-gray-500"
          >{{ detailedTime(item.time) }}</time
        >
        <br />
        <div v-if="!item.reviewer" class="italic">
          <i18n-t v-if="item.approve === null" tag="p" keypath="stb-request">
            <router-link :to="`/user/${uploader.id}`" class="link link-hover">{{
              uploader.name
            }}</router-link>
          </i18n-t>
        ...
        <div v-else-if="item.comment || item.comment === ''" class="mb-4">
          <i18n-t tag="span" keypath="comment" class="italic">
            <router-link
              :to="`/user/${item.reviewer}`"
              class="link link-hover"
              >{{ item.reviewerName }}</router-link
            >
        ...
        <i18n-t
          v-else
          tag="p"
          :keypath="item.approve ? 'stb-action.approve' : 'stb-action.deny'"
          class="mb-4 italic"
        >
          <router-link :to="`/user/${item.reviewer}`" class="link link-hover">{{
            item.reviewerName
          }}</router-link>
        </i18n-t>
      </li>
