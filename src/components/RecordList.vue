...
<template>
  <div
    class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 min-h-0 min-w-0"
  >
    ...
    >
      <figure v-if="record?.chartDetail">
        <img
          class="w-full aspect-[4/1] lg:aspect-[8/3]"
          :src="fileToURL(record?.chartDetail?.illustration) + '.thumbnail'"
        />
      </figure>
      <div
        class="card-body flex flex-row items-center justify-start p-4 gap-0 group aspect-[4/1] lg:aspect-[8/3] min-h-0 min-w-0"
      >
        <p
          class="font-tektur text-7xl text-center flex-none mx-2 w-12"
          :class="{
            '-mt-4': record.score >= 1000000,
            'drop-shadow-[0_0_4px_#1e7fff]':
              960000 <= record.score &&
              record.score < 1000000 &&
              record.full_combo,
        ...
        >
          ...
          <span
            v-else-if="record.score < 1000000"
            :class="{ 'text-[#1e7fff]': record.full_combo }"
            >V</span
          >
